import sys


def deconv(cip, key):
    key_list = list(key)
    cip_list = list(cip)

    len_k = len(key_list)
    len_c = len(cip_list)

    # len_c = len_p + len_k - 1
    len_p = len_c - len_k + 1

    # we need modular inv for the first byte of the key (\xab)
    key0_inv = pow(key_list[0],-1,256)

    plain_list = [0] * len_p

    for i in range(len_p):
        #sum(p[i-j] * k[j] for j in range(1,i))

        csum = 0

        for j in range(1, min(i+1, len_k)):
            csum += plain_list[i-j] * key_list[j]

        numerator = (cip_list[i] - (csum%256))%256
        plain_list[i] = (numerator * key0_inv) %256
    return bytes(plain_list)


key = b'\xab\xec\xe9<\xaaC\x7fr\xeb\x8dgQ\xc0\x94\x01\x1d\xc03\x14\x97\xe2\x91\x97\xcf\x8b\x13?\x1d24w|'
cip_hex = ''

cip = bytes.fromhex(cip_hex)

plain1 = deconv(cip,key)

print(plain1)